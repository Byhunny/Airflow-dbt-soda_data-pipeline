...
with DAG(
    ...
    stg_test = BashOperator(
        task_id="stg_test",
        bash_command=f"soda scan -d postgres_db -c {SODA_PATH}/configuration.yml {SODA_PATH}/stg_checks.yml",
        outlets=[Dataset(f"SEED://JAFFLE_SHOP")],
    )

    deps_install >> drop_seeds >> create_seeds >> stg_test